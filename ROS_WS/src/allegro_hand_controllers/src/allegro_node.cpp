...
AllegroNode::AllegroNode(bool sim /* = false */) {
  ...
  // states.
  joint_state_pub_1 = nh.advertise<sensor_msgs::JointState>(JOINT_STATE_TOPIC_1, 3);
  joint_state_pub_2 = nh.advertise<sensor_msgs::JointState>(JOINT_STATE_TOPIC_2, 3);
...
void AllegroNode::publishData() {
  ...
  joint_state_pub_1.publish(current_joint_state);
  joint_state_pub_2.publish(current_joint_state);
}
