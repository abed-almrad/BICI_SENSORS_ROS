...
print("Please enter the desired grasp attempt:")
grasp_attempt = input()
print("Please enter the desired iteration number:")
iter_num = input()
#Creating the numpy vector of the inactive points from a previously saved csv file
xyz = np.genfromtxt(grasp_attempt+'_inactive_points_coordinates_'+iter_num+'.csv',dtype= float, delimiter=',')
...
xyz = np.genfromtxt(grasp_attempt+'_active_points_coordinates_'+iter_num+'.csv',dtype= float, delimiter=',')
active_pts = xyz[:,0:3]
